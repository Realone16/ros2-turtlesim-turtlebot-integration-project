...
{
public:
    TurtleBotController(const rclcpp::NodeOptions &options) : LifecycleNode("turtlebot_controller", options) 
    {
        x_init = 5.0;
        y_init = 4.0;
...
private:
    ...
    void execute_goal(const std::shared_ptr<MoveTurtleGoalHandle> goal_handle)
    {
        ...
        //Publish on the cmd_vel topic for the duration_sec
        ...
        while (rclcpp::ok()){
            ...
            if (now() - start > duration){
                //last command to stop the turtle
                auto msg = Twist();
                msg.linear.x = 0;
                msg.angular.z = 0;
            ...
            //Condition if the goal was aborted because the server was deactivated 
            if (!server_activated_){
                auto msg = Twist();
                msg.linear.x = 0;
                msg.angular.z = 0;
...
//    auto node = std::make_shared<final_project::TurtleBotController>(); 
